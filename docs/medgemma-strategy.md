...
## 1. Context Length & VRAM Constraints

Operating multi-modal models like MedGemma requires careful management of context lengths to avoid Out-of-Memory (OOM) errors, especially on hardware with limited VRAM (e.g., 6GB or 8GB GPUs).
...
*   **Optimal Context (6144):** For environments with ~6GB VRAM, a context size of `6144` provides the optimal balance. It leaves sufficient headroom (approx. 1.4 GB on a 6GB card) for dynamic allocations during inference while supporting multi-image prompts.
*   **Context Extension (8192+):** Attempting to run at `-c 8192` or higher on constrained hardware can lead to OOM crashes due to ballooning compute buffers. If a larger context is strictly required, you must offload a portion of the LLM layers to system RAM (`--gpu-layers N`) to guarantee enough VRAM for image processing buffers.

### Future Explorations
*   **Quantization Tuning:** Exploring 8-bit quantized models (Q8_0) within the safe `-c 6144` boundary might yield improved diagnostic accuracy without hitting VRAM limits.
*   **Batch Image Contexts:** If running on larger GPUs (e.g., 12GB+), extending the context to `8192` or higher is highly recommended to support analyzing larger batches of images simultaneously.
...
*   **Slot Division (`-np`):** The `-np N` parameter divides the total allocated context memory (`-c`) into `N` independent slots. For example, `-c 6144 -np 3` creates 3 slots of 2048 tokens each, occupying the same memory footprint as a single 6144-token slot.
...
*   **VRAM Impact:** Processing multiple images simultaneously multiplies the CLIP compute requirements. Benchmarks confirm that 2 full-resolution images can be safely processed within a `-c 6144` context on a 6GB GPU.